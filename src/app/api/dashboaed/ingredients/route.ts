...
export async function POST(req: NextRequest) {
	try {
		...
		if (!data.name || data.name.trim().length === 0) {
			return NextResponse.json({ message: "Il nome dell'ingrediente è obbligatorio" }, { status: 400 });
		}
		...
		if (existingIngredient) {
			return NextResponse.json({ message: "Un ingrediente con questo nome esiste già" }, { status: 409 });
		}
