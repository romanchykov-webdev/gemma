...
export async function PATCH(req: NextRequest, context: { params: Promise<{ id: string }> }) {
	try {
		...
		if (!data.name || data.name.trim().length === 0) {
			return NextResponse.json({ message: "Il nome della categoria è obbligatorio" }, { status: 400 });
		}
		...
		if (duplicateCategory) {
			return NextResponse.json({ message: "Una categoria con questo nome esiste già" }, { status: 409 });
		}
