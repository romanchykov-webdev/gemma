...
export async function POST(req: Request) {
	try {
		...
		if (!previewImageUrl || typeof previewImageUrl !== "string") {
			return NextResponse.json({ message: "L'URL dell'immagine di anteprima è obbligatorio" }, { status: 400 });
		}

		if (!items || !Array.isArray(items) || items.length === 0) {
			return NextResponse.json({ message: "Almeno un elemento della storia è obbligatorio" }, { status: 400 });
		}

		// Валидация каждого item
		for (const item of items) {
			if (!item.sourceUrl || typeof item.sourceUrl !== "string") {
				return NextResponse.json({ message: "Tutti gli elementi devono avere un URL valido" }, { status: 400 });
			}
		}
