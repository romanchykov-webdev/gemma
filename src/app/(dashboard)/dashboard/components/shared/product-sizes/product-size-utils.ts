 ...
 */
export const validateProductSizeData = (data: CreateProductSizeData | UpdateProductSizeData): string | null => {
	if (!data.name.trim()) {
...
export const isDuplicateName = (
	...
	return sizes.some((size) => size.name.toLowerCase() === name.toLowerCase() && size.id !== excludeId);
};
